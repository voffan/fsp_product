...
const ProfileScreen = () => {
  ...
  const { data, isLoading: isContestsLoading } = useQuery({
    queryKey: ["contests"],
  ...
  if (isPending || isLoading || isContestsLoading)
    return (
  ...
  if (user)
    return (
      <div className="flex flex-col gap-8">
        ...
        >
          {/* Заголовок */}
          <div className="text-2xl">Мой профиль</div>
        ...
        {data && (
          <div className="flex flex-col gap-6 shadow-md border-darkgray border-[1px] rounded-3xl p-8">
            {/* Заголовок */}
            <div className="text-2xl">Мои подписки</div>

            <ContestTable
              isLoading={isContestsLoading}
              data={data.data.filter((item) =>
                subscriptions.find((sub) => sub.contest === item.id)
              )}
            />
          </div>
        )}
